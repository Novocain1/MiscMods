...
namespace WorldGenTests
{
    public class FractalNoise : NormalizedSimplexNoise
    {
        ...
        public override double Noise(double x, double y)
        {
            ...
            double f = 0.5000 * base.Noise(x, y);
            x = mat[0] * ox + mat[1] * oy;
            y = mat[2] * ox + mat[3] * oy;
            ox = x;
            oy = y;

            f += 0.2500 * base.Noise(x, y);
            x = mat[0] * ox + mat[1] * oy;
            y = mat[2] * ox + mat[3] * oy;
            ox = x;
            oy = y;

            f += 0.1250 * base.Noise(x, y);
            x = mat[0] * ox + mat[1] * oy;
            y = mat[2] * ox + mat[3] * oy;

            f += 0.0625 * base.Noise(x, y);

            return f;
        }

        public new virtual double Noise(double x, double y, double[] amplitudes)
        {
            double ox = x, oy = y;

            double f = 0.5000 * base.Noise(x, y, amplitudes);
            x = mat[0] * ox + mat[1] * oy;
            y = mat[2] * ox + mat[3] * oy;
            ox = x;
            oy = y;

            f += 0.2500 * base.Noise(x, y, amplitudes);
            x = mat[0] * ox + mat[1] * oy;
            y = mat[2] * ox + mat[3] * oy;
            ox = x;
            oy = y;

            f += 0.1250 * base.Noise(x, y, amplitudes);
            x = mat[0] * ox + mat[1] * oy;
            y = mat[2] * ox + mat[3] * oy;
